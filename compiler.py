...
class LeanCompiler:
    ...
    # lean_ws 目录（带 Mathlib 的 lake 项目）
    _WS_DIR = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "lean_ws"
    )
    ...
    def _parse_lean_output(self, output: str, returncode: int) -> CompilationResult:
        ...
        for line in output.splitlines():
            ...
            if "error:" in line.lower():
                errors.append(line)
                error_explanations.append(self._explain_error(line))
            elif "warning:" in line.lower():
                warnings.append(line)
                warning_explanations.append(self._explain_warning(line))
